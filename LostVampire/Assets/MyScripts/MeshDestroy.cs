...
public class MeshDestroy : MonoBehaviour
{
    ...
    public void PreDestroy()
    {
        for (var i = 0; i < fragments.Count; i++)
        {
            ...
            fragments[i].GetComponent<Rigidbody>().AddForceAtPosition(fragments[i].transform.position * ExplodeForce, transform.position);
            if (!persistence) {
                Destroy(fragments[i], 2.5f);
            }
          //  Debug.Log(fragments[i]);
